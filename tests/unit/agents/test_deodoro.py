...
class TestAgentContext:
    ...
    def test_context_to_dict(self):
        ...
        context = AgentContext(
            investigation_id="test-123", user_id="user-test", metadata={"key": "value"}
        )
...
class TestAgentMessage:
    ...
    def test_message_defaults(self):
        ...
        message = AgentMessage(
            sender="sender", recipient="recipient", action="test_action"
        )
...
class TestBaseAgent:
    ...
    def test_agent_validation(self, agent):
        ...
        valid_message = AgentMessage(
            sender="sender", recipient=agent.name, action="test"
        )

        invalid_message = AgentMessage(
            sender="sender", recipient=agent.name, action="invalid_action"
        )
    ...
    async def test_agent_timeout_handling(self, agent, agent_context):
        ...
        async def slow_process(message, context):
            ...
            return AgentResponse(
                agent_name=timeout_agent.name, status=AgentStatus.COMPLETED
            )

        timeout_agent.process = slow_process

        message = AgentMessage(
            sender="sender", recipient=timeout_agent.name, action="test"
        )
...
class ConcreteReflectiveAgent(ReflectiveAgent):
    ...
    async def process(
        ...
        return AgentResponse(
            agent_name=self.name, status=AgentStatus.COMPLETED, result=result
        )

    async def _reflect_on_result(
        self, result: dict, original_message: AgentMessage
    ) -> dict:
...
class TestReflectiveAgent:
    ...
    async def test_process_without_reflection(self, reflective_agent, agent_context):
        ...
        response = await reflective_agent.process_with_reflection(
    ...
    async def test_reflection_iteration_limit(self, reflective_agent, agent_context):
        ...
        message = AgentMessage(
            sender="sender", recipient=always_reflect_agent.name, action="process"
        )

        response = await always_reflect_agent.process_with_reflection(
            message, agent_context
        )
...
class TestAgentIntegration:
    ...
    async def test_agent_communication(self):
        ...
        agent_a = ConcreteAgent(
            name="agent_a", description="First agent", capabilities=["sending"]
        )

        agent_b = ConcreteAgent(
            name="agent_b", description="Second agent", capabilities=["receiving"]
        )
