...
from src.infrastructure.retry import ExponentialBackoff, RetryManager, RetryPolicy
...
class TestMetricsCollector:
    ...
    def test_counter_metric(self, metrics_collector):
        ...
        metrics_collector.increment(
            "api_requests_total", labels={"endpoint": "/health"}
        )
        metrics_collector.increment(
            "api_requests_total", labels={"endpoint": "/health"}
        )
        metrics_collector.increment(
            "api_requests_total", labels={"endpoint": "/api/v1/users"}
        )
    ...
    def test_histogram_metric(self, metrics_collector):
        ...
        for duration in durations:
            metrics_collector.record_duration(
                "request_duration_seconds", duration, labels={"method": "GET"}
            )
    ...
    def test_metric_labels(self, metrics_collector):
        ...
        db_errors = metrics_collector.get_metric_value(
            "errors_total", labels={"type": "database"}
        )
        api_errors = metrics_collector.get_metric_value(
            "errors_total", labels={"type": "api"}
        )
...
class TestPerformanceMonitor:
    ...
    def test_throughput_calculation(self, perf_monitor):
        ...
        throughput = perf_monitor.calculate_throughput(
            "process_request", window_seconds=1
        )
...
class TestRetryPolicy:
    ...
    def test_exponential_backoff(self):
        ...
        backoff = ExponentialBackoff(initial_delay=1, max_delay=60, multiplier=2)
...
class TestDatabasePool:
    ...
    def db_pool(self):
        ...
        return DatabasePool(min_size=2, max_size=10, max_idle_time=300)
...
class TestMessageQueue:
    ...
    async def test_message_persistence(self, message_queue):
        ...
        message_id = await message_queue.publish(
            "persistent.topic", {"important": "data"}, persistent=True
        )
