...
class PatternAnalyzer(MLModel):
    ...
    async def evaluate(self, data: list[dict]) -> dict:
        ...
        return {
            ...
            "significant_patterns": len(
                [p for p in patterns if p["significance"] > 0.6]
            ),
        }
    ...
    def _analyze_value_patterns(self, data: list[dict]) -> dict:
        ...
        return {
            ...
            "quartiles": {"q1": values[n // 4], "q3": values[3 * n // 4]},
    ...
    def _find_peak_periods(self, monthly_data: dict) -> list[str]:
        ...
        return [
            month for month, value in monthly_data.items() if value > avg_spending * 1.5
        ]
    ...
    def _calculate_concentration(self, supplier_values: dict) -> float:
        ...
        concentration = sum(
            (value / total_value) ** 2 for value in supplier_values.values()
        )
    ...
    def _calculate_distribution(self, category_data: dict) -> dict:
        ...
        return {
            category: (value / total) * 100 for category, value in category_data.items()
        }
