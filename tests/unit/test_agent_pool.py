...
from src.agents.deodoro import AgentMessage, AgentStatus, BaseAgent
...
class TestAgentInstance:
    ...
    async def test_agent_instance_health_check(self, mock_agent):
        ...
        instance = AgentInstance(agent=mock_agent, agent_type=AgentType.INVESTIGATOR)
...
class TestAgentPool:
    ...
    def pool_config(self):
        ...
        return AgentPoolConfig(
            min_agents_per_type=2, max_agents_per_type=5, enable_auto_scaling=True
        )
    ...
    async def test_concurrent_agent_requests(self, agent_pool):
        ...
        tasks = []
        for _ in range(10):
            task = asyncio.create_task(agent_pool.get_agent(AgentType.INVESTIGATOR))
            tasks.append(task)
...
class TestAgentPoolRecovery:
    ...
    def recovery_pool(self):
        ...
        config = AgentPoolConfig(
            min_agents_per_type=2, max_retries=3, enable_circuit_breaker=True
        )
    ...
    async def test_agent_crash_recovery(self, recovery_pool):
        ...
        message = AgentMessage(sender="test", recipient=agent.agent_id, action="test")
