...
from ..websocket import WebSocketMessage, connection_manager, websocket_handler
...
async def investigation_websocket(
    ...
    await websocket_manager.join_room(
        connection_id, f"investigation:{investigation_id}"
    )

    # Connect and subscribe with legacy manager
    await connection_manager.connect(
        websocket, user_id, f"investigation_{investigation_id}"
    )
    ...
    try:
        ...
        logger.info(
            f"Investigation WebSocket disconnected: user_id={user_id}, investigation_id={investigation_id}"
        )
        ...
        await websocket_manager.disconnect(connection_id)
        await connection_manager.unsubscribe_from_investigation(
...
async def analysis_websocket(
    ...
    try:
        ...
        logger.info(
            f"Analysis WebSocket disconnected: user_id={user_id}, analysis_id={analysis_id}"
        )
