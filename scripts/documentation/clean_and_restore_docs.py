...
def clean_mdx_content(content: str) -> str:
    ...
    body = re.sub(
        r'<span[^>]*style="[^"]*"[^>]*>(.*?)</span>', r"\1", body, flags=re.DOTALL
    )
...
def restore_documentation():
    ...
    source_base = Path(
        "/home/anderson-henrique/Documentos/cidadao.ai-backend/docs_new/docs_problematic"
    )
    target_base = Path(
        "/home/anderson-henrique/Documentos/cidadao.ai-backend/docs_new/docs"
    )
