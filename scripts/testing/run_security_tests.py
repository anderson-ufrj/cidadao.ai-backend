...
def main():
    ...
    if args.test_type in ["security", "all"]:
        ...
        total_tests += 1
        if run_command(
            f"{pytest_cmd} tests/security/test_oauth.py", "OAuth2 Security Tests"
        ):
            success_count += 1
        ...
        total_tests += 1
        if run_command(
            f"{pytest_cmd} tests/security/test_audit.py", "Audit Logging Tests"
        ):
            success_count += 1
    ...
    if args.test_type in ["unit", "all"]:
        ...
        total_tests += 1
        if run_command(f"{pytest_cmd} tests/unit/", "Unit Tests"):
            success_count += 1
    ...
    if args.test_type in ["integration", "all"]:
        ...
        total_tests += 1
        if run_command(f"{pytest_cmd} tests/integration/", "Integration Tests"):
            success_count += 1
    ...
    if args.test_type in ["security", "all"]:
        ...
        total_tests += 1
        if run_command(
            "python -m safety check", "Safety - Known Vulnerabilities Check"
        ):
            success_count += 1
        ...
        total_tests += 1
        if run_command(
            "python -m mypy src/ --ignore-missing-imports", "MyPy - Type Checking"
        ):
            success_count += 1
