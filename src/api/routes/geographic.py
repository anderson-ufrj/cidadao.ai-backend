...
class GeographicBoundary(BaseModel):
    ...
    properties: dict[str, Any] = Field(
        default_factory=dict, description="Collection properties"
    )
...
async def get_geographic_boundaries(
    ...
    simplified: bool = Query(
        True, description="Return simplified boundaries for performance"
    ),
    ...
    try:
        ...
        if cached_data:
            logger.info(
                "Returning cached geographic boundaries", region_type=region_type.value
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve boundaries: {str(e)}"
        )
...
async def get_region_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get region details: {str(e)}"
        )
...
async def get_geographic_data(
    ...
    region_type: RegionType = Query(
        RegionType.STATE, description="Geographic aggregation level"
    ),
    ...
    try:
        ...
        for metric_data in regional_data.metrics:
            data_points.append(
                RegionalDataPoint(
                    ...
                    normalized_value=(
                        metric_data.normalized_value if normalize else None
                    ),
                    metadata={
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve geographic data: {str(e)}"
        )
...
async def get_region_coordinates(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Coordinates not found for region '{region_id}'"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get coordinates: {str(e)}"
        )
