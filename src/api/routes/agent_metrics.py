...
async def get_all_agents_summary(current_user: User = Depends(get_current_user)):
...
@router.post("/reset")
async def reset_all_metrics(current_user: User = Depends(get_current_user)):
    """Reset metrics for all agents."""
    try:
        await agent_metrics_service.reset_metrics()

        return {"status": "success", "message": "All agent metrics have been reset"}
...
async def metrics_health_check():
    ...
    try:
        ...
        return {"status": "unhealthy", "service": "agent_metrics", "error": str(e)}
