...
class TestAnalysisService:
    ...
    @pytest.mark.asyncio
    async def test_anomaly_detection_in_contracts(
        self, analysis_service, sample_contracts
    ):
        ...
        spending_data = pd.DataFrame(
            {
                ...
                "department": np.random.choice(
                    ["Health", "Education", "Infrastructure"], len(dates)
                ),
            }
        )
    ...
    @pytest.mark.asyncio
    async def test_vendor_concentration_analysis(
        self, analysis_service, sample_contracts
    ):
    ...
    async def test_risk_assessment(self, analysis_service, sample_contracts):
        ...
        risk_score = await assessment.calculate_risk(
            contracts=risk_contracts, historical_data=sample_contracts
        )
...
class TestDataService:
    ...
    async def test_data_filtering(self, data_service):
        ...
        # Apply filters
        filters = DataFilter(
            entity="MinHealth", status="active", date_range=("2024-01-01", "2024-12-31")
        )
...
class TestInvestigationService:
    ...
    async def test_execute_investigation(self, investigation_service):
        ...
        with patch("src.agents.abaporu.MasterAgent.execute") as mock_execute:
            ...
            request = InvestigationRequest(
                id="inv-exec-123", query="Test investigation", parameters={}
            )
    ...
    async def test_investigation_caching(self, investigation_service):
        ...
        with patch("src.agents.abaporu.MasterAgent.execute") as mock_execute:
            mock_execute.return_value = AsyncMock(
                status="completed", result={"data": "first_execution"}
            )
