...
class TransparencyAnalysisRequest(BaseModel):
    ...
    include_explanation: bool = Field(
        default=True, description="Incluir explicação detalhada dos resultados"
    )
    confidence_threshold: float = Field(
        default=0.7, description="Limiar de confiança para alertas", ge=0.0, le=1.0
    )
...
class ModelInfoResponse(BaseModel):
    ...
    training_data: dict = Field(
        ..., description="Informações sobre dados de treinamento"
    )
...
class CidadaoAIManager:
    ...
    async def analyze_transparency(
        self, request: TransparencyAnalysisRequest
        ...
        try:
            ...
            executive_summary, recommendations, overall_confidence = (
                self._generate_summary(results, request.confidence_threshold)
            )
    ...
    async def chat_completion(self, request: ChatRequest) -> ChatResponse | Generator:
        ...
        try:
            ...
            is_transparency_query = any(
                keyword in user_message.lower() for keyword in transparency_keywords
            )

            if is_transparency_query:
                # Usar análise especializada
                analysis_request = TransparencyAnalysisRequest(
                    text=user_message, analysis_type="complete"
                )
    ...
    def _generate_summary(
        self, results: dict, confidence_threshold: float
    ) -> Tuple[dict, list[str], float]:
        """Gerar resumo executivo e recomendações"""

        summary = {"overall_risk": "Baixo", "main_findings": [], "alert_level": "Verde"}
        ...
        if "anomaly_detection" in results:
            ...
            if anomalous_count > 0:
                summary["main_findings"].append(
                    f"{anomalous_count} anomalias detectadas"
                )
                summary["alert_level"] = "Vermelho"
                summary["overall_risk"] = "Alto"
                recommendations.append(
                    "🚨 Investigação imediata necessária devido a anomalias detectadas"
                )
        ...
        if "financial_analysis" in results:
            ...
            if high_risk_count > 0:
                summary["main_findings"].append(
                    f"{high_risk_count} contratos de alto risco financeiro"
                )
                if summary["overall_risk"] == "Baixo":
                    summary["overall_risk"] = "Médio"
                    summary["alert_level"] = "Amarelo"
                recommendations.append(
                    "⚠️ Revisão financeira recomendada para contratos de alto risco"
                )

            if avg_value > 10000000:  # > 10M
                summary["main_findings"].append(
                    f"Valor médio elevado: R$ {avg_value:,.2f}"
                )
        ...
        if "legal_compliance" in results:
            ...
            if compliance_rate < 0.8:
                summary["main_findings"].append(
                    f"Taxa de conformidade baixa: {compliance_rate:.1%}"
                )
                recommendations.append(
                    "📋 Revisão de processos de compliance necessária"
                )
    ...
    def _format_analysis_for_chat(self, analysis: TransparencyAnalysisResponse) -> str:
        ...
        response_parts.append("📊 **Análise de Transparência**")
    ...
    def _generate_general_response(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["olá", "oi", "bom dia", "boa tarde"]):
            return (
                "Olá! Sou o Cidadão.AI, seu assistente de IA especializado em transparência pública brasileira. "
    ...
    def _convert_to_csv(self, results: list[TransparencyAnalysisResponse]) -> str:
        ...
        for result in results:
            ...
            if result.anomaly_detection:
                row["anomalous_count"] = result.anomaly_detection["summary"][
                    "anomalous_count"
                ]

            # Adicionar detalhes financeiros
            if result.financial_analysis:
                row["high_risk_count"] = result.financial_analysis["summary"][
                    "high_risk_count"
                ]
                row["avg_estimated_value"] = result.financial_analysis["summary"][
                    "average_estimated_value"
                ]

            # Adicionar conformidade legal
            if result.legal_compliance:
                row["compliance_rate"] = result.legal_compliance["summary"][
                    "compliance_rate"
                ]
...
@app.get(
    "/model/info", response_model=ModelInfoResponse, summary="Informações do Modelo"
)
...
@app.post(
    "/analyze",
    response_model=TransparencyAnalysisResponse,
    summary="Análise de Transparência",
)
...
async def batch_analyze(request: BatchAnalysisRequest):
    ...
    if request.format == "csv":
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": "attachment; filename=cidadao_analysis.csv"
            },
        )
...
async def upload_file(file: UploadFile = File(...)):
    ...
    if not file.filename.endswith((".csv", ".txt", ".json")):
        raise HTTPException(
            status_code=400, detail="Formato não suportado. Use CSV, TXT ou JSON."
        )
    ...
    try:
        ...
        batch_request = BatchAnalysisRequest(
            texts=texts, analysis_type="complete", format="json"
        )
        ...
        "chat": {
            ...
            "example": {
                "messages": [
                    {
                        "role": "user",
                        "content": "Analise este contrato: Aquisição de medicamentos por R$ 5 milhões sem licitação.",
                    }
                ]
            },
        },
...
if __name__ == "__main__":
    ...
    # Executar servidor
    uvicorn.run(
        "src.ml.model_api:app", host="0.0.0.0", port=8001, reload=True, log_level="info"
    )
