...
class TestRunner:
    ...
    def run_linting(self) -> Dict:
        ...
        results["mypy"] = self.run_command(
            ["python", "-m", "mypy", "src/"], "Running type checking (MyPy)"
        )
...
@click.option(
    "--integration-only", "-i", is_flag=True, help="Run only integration tests"
)
...
@click.option(
    "--coverage-threshold", "-t", default=80, help="Coverage threshold percentage"
)
@click.option("--fast", "-f", is_flag=True, help="Skip slower checks")
def main(
    unit_only, integration_only, agents_only, quality_only, coverage_threshold, fast
):
    ...
    console.print(
        f"[bold cyan]🚀 Starting test execution in: {project_root}[/bold cyan]"
    )
    ...
    if unit_only:
        ...
        runner.display_results_table(
            {"Integration Tests": result}, "Integration Test Results"
        )
        ...
        if not success:
            console.print(
                "[red]❌ Some tests failed. Please review the results above.[/red]"
            )
            sys.exit(1)
        else:
