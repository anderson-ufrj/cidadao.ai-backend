...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _clean_old_requests(self, client_data: dict[str, list], current_time: float):
        ...
        # Clean minute window
        client_data["minute"] = [
            t for t in client_data["minute"] if current_time - t < 60
        ]

        # Clean hour window
        client_data["hour"] = [
            t for t in client_data["hour"] if current_time - t < 3600
        ]

        # Clean day window
        client_data["day"] = [t for t in client_data["day"] if current_time - t < 86400]
