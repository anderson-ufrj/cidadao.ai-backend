...
def niemeyer_agent(mock_infrastructure_service):
    with patch(
        "src.agents.niemeyer.InfrastructureService",
        return_value=mock_infrastructure_service,
    ):
...
class TestNiemeyerAgent:
    ...
    async def test_infrastructure_assessment(self, niemeyer_agent):
        ...
        assert (
            response.result["infrastructure_assessment"]["investment_efficiency"]
            == 0.67
        )
    ...
    async def test_architectural_quality_analysis(self, niemeyer_agent):
        ...
        message = AgentMessage(
            sender="test",
            recipient="Niemeyer",
            action="analyze_architectural_quality",
