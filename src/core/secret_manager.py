...
class SecretSchema(BaseModel):
    ...
    def dict_safe(self, **kwargs) -> dict[str, Any]:
        ...
        for field_name, field in self.__fields__.items():
            if field.type_ == SecretStr or (
                hasattr(field.type_, "__origin__")
                and field.type_.__origin__ is SecretStr
            ):
...
class SecretManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                ...
                vault_status=(
                    self._vault_client._status.value
    ...
    async def get_secret(
        ...
        try:
            # Try Vault first
            if self._vault_client and self._vault_client._status in [
                VaultStatus.HEALTHY,
            ...
            if env_value is not None:
                ...
                return SecretResult(
                    value=env_value, source=SecretSource.ENVIRONMENT, key=key
                )

            # Use default if provided
            if default is not None:
                logger.debug(
                    "secret_using_default", key=key, has_default=default is not None
                )

                return SecretResult(value=default, source=SecretSource.DEFAULT, key=key)
            ...
            logger.error("secret_retrieval_error", key=key, error=str(e))
    ...
    async def set_secret(
        ...
        try:
            ...
            if success:
                logger.info("secret_stored", key=key, has_metadata=metadata is not None)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        if self._vault_client:
            ...
            status.update(
                {"vault_status": vault_stats["status"], "vault_stats": vault_stats}
            )
