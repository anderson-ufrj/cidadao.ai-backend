...
from src.infrastructure.health.dependency_checker import HealthStatus, health_manager
...
async def get_detailed_health_status(
    ...
    run_checks: bool = Query(
        default=True, description="Whether to run fresh health checks"
    ),
):
    """
    ...
    try:
        ...
        return {
            **health_report,
            ...
            "check_duration_ms": (
                sum(result.response_time_ms for result in results.values())
                if run_checks
                else 0
            ),
        }
...
async def get_dependency_health(
    ...
    try:
        ...
        if dependency_name in health_manager.last_check_results:
            ...
            raise HTTPException(
                status_code=404, detail=f"Dependency '{dependency_name}' not found"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve dependency health"
        )
...
async def get_slo_status(current_user=Depends(get_current_user)):
    """
...
async def get_specific_slo_status(
    slo_name: str, current_user=Depends(get_current_user)
):
    """
...
async def create_slo_target(
    request: SLOTargetRequest, current_user=Depends(get_current_user)
):
    """
...
async def record_slo_metric(
    request: MetricRecordRequest, current_user=Depends(get_current_user)
):
    """
...
async def get_error_budget_report(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve error budget report"
        )
...
async def test_api_performance(
    ...
    requests_count: int = Query(
        default=10, ge=1, le=100, description="Number of requests to send"
    ),
    current_user=Depends(get_current_user),
):
    """
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i, response in enumerate(responses):
                if isinstance(response, Exception):
                    ...
                    # Record success
                    record_api_request(
                        endpoint, response_time_ms, response.status_code < 400
                    )
...
async def get_monitoring_dashboard_summary(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        dependency_health_score = (
            (healthy_dependencies / total_dependencies * 100)
        ...
        return {
            ...
            "error_budgets": {
                ...
                "critical_budgets": sum(
                    1
                    for budget in error_budget["error_budgets"].values()
            ...
            "performance_indicators": {
                "avg_health_check_time": (
                    sum(result.response_time_ms for result in health_results.values())
                    / len(health_results)
                    if health_results
                    else 0
                ),
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve monitoring summary"
        )
