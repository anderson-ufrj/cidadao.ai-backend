...
class TestChatEmergencyEndpoint:
    ...
    def test_emergency_endpoint_greeting(self, client):
        ...
        response = client.post(
            "/api/v1/chat/emergency", json={"message": "Olá, bom dia!"}
        )
    ...
    def test_emergency_endpoint_help(self, client):
        ...
        response = client.post(
            "/api/v1/chat/emergency", json={"message": "Como posso usar o sistema?"}
        )
        ...
        assert any(
            action in ["help", "examples", "documentation"]
    ...
    def test_emergency_endpoint_without_session_id(self, client):
        ...
        response = client.post("/api/v1/chat/emergency", json={"message": "teste"})
    ...
    async def test_emergency_endpoint_with_maritaca(self, client):
        ...
        with patch.dict("os.environ", {"MARITACA_API_KEY": "test-key"}):
            with patch("src.api.routes.chat_emergency.try_maritaca") as mock_maritaca:
                ...
                response = client.post(
                    "/api/v1/chat/emergency", json={"message": "teste com maritaca"}
                )
    ...
    def test_emergency_endpoint_validation_error(self, client):
        ...
        # Empty message
        response = client.post("/api/v1/chat/emergency", json={"message": ""})
        assert response.status_code == 422

        # Message too long
        response = client.post("/api/v1/chat/emergency", json={"message": "a" * 1001})
        assert response.status_code == 422

        # Missing message
        response = client.post("/api/v1/chat/emergency", json={})
        assert response.status_code == 422
