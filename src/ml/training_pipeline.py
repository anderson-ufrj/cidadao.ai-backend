...
class MLTrainingPipeline:
    ...
    def _initialize_mlflow(self):
        ...
        try:
            mlflow.set_tracking_uri(
                getattr(settings, "MLFLOW_TRACKING_URI", "file:./mlruns")
            )
    ...
    async def train_model(
        ...
        try:
            ...
            with mlflow.start_run(
                run_name=f"{model_type}_{algorithm}_{datetime.now().isoformat()}"
            ):
                # Log parameters
    ...
    async def _create_model(
        ...
        default_params = {
            ...
            "one_class_svm": {"gamma": 0.001, "nu": 0.05, "kernel": "rbf"},
    ...
    async def _create_model_version(
        self, model_type: str, algorithm: str, model_path: str, metrics: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def load_model(
        ...
        if version is None:
            ...
            prod_versions = [
                v for v in registry["versions"] if v.get("status") == "production"
            ]
            ...
            version_data = next(
                (v for v in registry["versions"] if v["version"] == version), None
            )
    ...
    async def promote_model(
        self, model_id: str, version: int, status: str = "production"
    ) -> bool:
        """
        ...
        try:
            ...
            await redis_client.set(
                f"ml_model_registry:{model_id}", json.dumps(registry), ex=86400 * 30
            )
    ...
    async def compare_models(
        ...
        for model_id, version in model_ids:
            try:
                ...
                results[f"{model_id}_v{version or 'latest'}"] = {"error": str(e)}
