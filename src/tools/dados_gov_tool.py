...
class DadosGovTool:
    ...
    async def _search_datasets(
        ...
        for dataset in result.results:
            dataset_info = {
                ...
                "title": dataset.title,
                "organization": (
                    dataset.organization.title if dataset.organization else "Unknown"
                ),
                "description": (
                    dataset.notes[:200] + "..."
                    if dataset.notes and len(dataset.notes) > 200
                    else dataset.notes
                ),
                "resources": [
                    {
    ...
    async def _get_dataset(self, dataset_id: str) -> ToolResult:
        ...
        details = {
            ...
            "created": (
                dataset.metadata_created.isoformat()
                if dataset.metadata_created
                else None
            ),
            "modified": (
                dataset.metadata_modified.isoformat()
            ...
            "resources": [
                {
                    ...
                    "size": res.size,
                    "last_modified": (
                        res.last_modified.isoformat() if res.last_modified else None
                    ),
                }
            ...
            spending_data.append(
                {
                    ...
                    "formats": list(
                        set(res.format for res in dataset.resources if res.format)
                    ),
                }
            )
