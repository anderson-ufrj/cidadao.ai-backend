...
class CidadaoAIModel(PreTrainedModel):
    ...
    def forward(
        ...
        if corruption_indicators is not None:
            corruption_embeds = self.transparency_embeddings["corruption_indicators"](
...
class TransparencyAnalysisPipeline(Pipeline):
    ...
    def postprocess(self, model_outputs, return_all_scores=False):
        ...
        if (
            hasattr(model_outputs, "anomaly_logits")
            or "anomaly_logits" in model_outputs
        ):
            anomaly_logits = model_outputs.get(
                "anomaly_logits", model_outputs.anomaly_logits
            )
                ...
                "all_scores": (
                    [
                        ...
                        for label, score in zip(
                            self.anomaly_labels, anomaly_probs[0], strict=False
                        )
                    ]
        ...
        # Análise financeira
        if (
            hasattr(model_outputs, "financial_logits")
            or "financial_logits" in model_outputs
        ):
            financial_logits = model_outputs.get(
                "financial_logits", model_outputs.financial_logits
            )
            ...
            results["financial"] = {
                ...
                "all_scores": (
                    [
                        {"label": label, "score": score.item()}
                        for label, score in zip(
                            self.financial_labels, financial_probs[0], strict=False
                        )
                    ]
                    if return_all_scores
                    else None
            ...
            results["legal"] = {
                ...
                "all_scores": (
                    [
                        {"label": label, "score": score.item()}
                        for label, score in zip(
                            self.legal_labels, legal_probs[0], strict=False
                        )
                    ]
                    if return_all_scores
                    else None
...
def create_cidadao_pipeline(
    ...
    **kwargs,
    ...
    return TransparencyAnalysisPipeline(model=model, tokenizer=tokenizer, task=task)


# Função de conveniência para uso rápido
def analyze_transparency(
    text: str, model_name: str = "neural-thinker/cidadao-gpt"
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"✅ Modelo Cidadão.AI criado com {sum(p.numel() for p in model.parameters()):,} parâmetros"
    )
    print("🎯 Tarefas habilitadas: Anomalias, Financeiro, Legal")
    ...
    print(
        f"🔍 Anomaly logits: {outputs.anomaly_logits.shape if 'anomaly_logits' in outputs else 'N/A'}"
    )
    print(
        f"💰 Financial logits: {outputs.financial_logits.shape if 'financial_logits' in outputs else 'N/A'}"
    )
    print(
        f"⚖️ Legal logits: {outputs.legal_logits.shape if 'legal_logits' in outputs else 'N/A'}"
    )
