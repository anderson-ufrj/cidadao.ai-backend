...
async def initialize_memory_system() -> ContextMemoryAgent | None:
    ...
    try:
        ...
        memory_integration.auto_retrieve = getattr(
            settings, "AUTO_RETRIEVE_MEMORIES", True
        )
        ...
        logger.warning(
            "Continuing without memory system - agents will operate independently"
        )
        return None
        ...
        for agent_type, pool_entries in agent_pool._pools.items():
            for entry in pool_entries:
                try:
                    ...
                    logger.error(
                        f"Failed to integrate agent {entry.agent.agent_id}: {str(e)}"
                    )
...
async def demonstrate_memory_sharing():
    ...
    try:
        ...
        success = await memory_integration.share_knowledge_between_agents(
            source_agent="oxossi", target_agent="zumbi", knowledge_type="fraud"
        )
