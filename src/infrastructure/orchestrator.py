...
from .agent_pool import cleanup_agent_pool, get_agent_pool_manager
from .cache_system import cleanup_cache, get_cache_manager

# Import all infrastructure components
from .database import cleanup_database, get_database_manager
from .monitoring_service import cleanup_monitoring, get_monitoring_manager

# Import ML components
try:
    from ..ml.advanced_pipeline import MLPipelineManager, get_ml_pipeline_manager
    from ..ml.hf_integration import CidadaoAIHubManager, get_cidadao_manager
...
class CidadaoAIOrchestrator:
    ...
    async def _initialize_sequential(self) -> bool:
        ...
        for component_name in self._initialization_order:
            ...
            if component_name == "monitoring" and self.config.enable_monitoring:
                success = await self._init_component(
                ...
                and self.config.enable_ml_pipeline
                and ML_AVAILABLE
            ):
                success = await self._init_component(
                ...
                component_name == "cidadao_gpt"
                and self.config.enable_cidadao_gpt
                and ML_AVAILABLE
            ):
                ...
                success = await self._init_component(
                    "agent_pool", get_agent_pool_manager
                )
            else:
                continue
    ...
    async def _init_component(self, name: str, factory_func) -> bool:
        ...
        self.component_health[name] = ComponentHealth(
            name=name, status=ComponentStatus.INITIALIZING
        )
            ...
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    if attempt < self.config.max_retries - 1:
                        ...
                        logger.error(
                            f"❌ {name} falhou após {self.config.max_retries} tentativas"
                        )
                        return False
    ...
    async def _setup_agent_factories(self):
        ...
        try:
            ...
            agent_pool.register_agent_factory(
                "investigator", self._create_investigator_agent
            )
        ...
        try:
            if hasattr(component, "health_check"):
                ...
                if isinstance(health_result, dict):
                    # Parse health result
                    overall_status = health_result.get("overall", {}).get(
                        "status", "unknown"
                    )
    ...
    async def _update_system_status(self):
        ...
        if new_status != self.status:
            logger.info(
                f"📊 Status do sistema: {self.status.value} → {new_status.value}"
            )
    ...
    async def get_system_health(self) -> dict[str, Any]:
        ...
        health = {
            ...
            "summary": {
                ...
                "avg_health_score": (
                    sum(h.health_score for h in self.component_health.values())
    ...
    async def submit_investigation(self, query: str, **kwargs) -> str:
        ...
        task_id = await agent_pool.submit_task("master", "investigate", query, **kwargs)
        ...
        try:
            ...
            if hasattr(component, "shutdown"):
                await asyncio.wait_for(
                    component.shutdown(), timeout=self.config.shutdown_timeout
                )
            elif hasattr(component, "cleanup"):
                await asyncio.wait_for(
                    component.cleanup(), timeout=self.config.shutdown_timeout
                )
...
if __name__ == "__main__":
    ...
    async def test_orchestrator():
        ...
        try:
            ...
            print(
                f"📊 Componentes: {health['summary']['healthy_components']}/{health['summary']['total_components']} saudáveis"
            )
