...
class AnalysisRequest(BaseModel):
    ...
    data_source: str = PydanticField(default="contracts", description="Data source to analyze")
    ...
    include_correlations: bool = PydanticField(default=True, description="Include correlation analysis")
    include_trends: bool = PydanticField(default=True, description="Include trend analysis")
    include_predictions: bool = PydanticField(default=False, description="Include predictive analysis")
    
    @validator('analysis_type')
    def validate_analysis_type(cls, v):
        """Validate analysis type."""
        allowed_types = [
            'spending_trends', 'vendor_patterns', 'organizational_behavior',
            'seasonal_analysis', 'efficiency_metrics', 'correlation_analysis'
        ]
...
async def get_spending_trends(
    ...
    time_period: str = Query("6months", description="Time period (3months, 6months, 1year, 2years)"),
...
async def get_correlations(
    ...
    """
    if len(variables) < 2:
        raise HTTPException(status_code=400, detail="At least 2 variables required for correlation")
...
async def detect_patterns(
    ...
    """
    allowed_patterns = ["all", "vendor", "temporal", "value", "geographic", "organizational"]
    if pattern_type not in allowed_patterns:
        raise HTTPException(status_code=400, detail=f"Pattern type must be one of: {allowed_patterns}")
...
async def list_analyses(
    ...
    user_analyses = [
        analysis for analysis in _active_analyses.values()
        if analysis["user_id"] == user_id
    ]
    
    # Filter by analysis type if provided
    if analysis_type:
        user_analyses = [analysis for analysis in user_analyses if analysis["analysis_type"] == analysis_type]
    
    # Filter by status if provided
    if status:
        user_analyses = [analysis for analysis in user_analyses if analysis["status"] == status]
