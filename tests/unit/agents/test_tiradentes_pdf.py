...
""",
    ...
    async def test_render_pdf_basic(
        ...
        result = await reporter_agent._render_pdf(
            sections=sample_sections, request=request, context=agent_context
        )
    ...
    async def test_pdf_format_in_renderers(self, reporter_agent):
        ...
        assert (
            reporter_agent.format_renderers[ReportFormat.PDF]
            == reporter_agent._render_pdf
        )
    ...
    async def test_render_pdf_markdown_conversion(
        ...
        sections = [
            ReportSection(title="Test Section", content="Test content", importance=5)
        ]
    ...
    async def test_render_pdf_error_handling(
        ...
        mock_export_service.generate_pdf.side_effect = Exception(
    ...
    async def test_render_pdf_empty_sections(
        ...
        result = await reporter_agent._render_pdf(
            sections=[], request=request, context=agent_context
        )
    ...
    async def test_render_pdf_large_content(
        ...
        # Render PDF
        result = await reporter_agent._render_pdf(
            sections=large_sections, request=request, context=agent_context
        )
