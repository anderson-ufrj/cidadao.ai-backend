...
from src.api.auth import User, auth_manager, get_current_user, require_admin
...
async def get_oauth_authorization_url(
    ...
    try:
        ...
        return OAuthUrlResponse(
            authorization_url=authorization_url, state=state, provider=provider.value
        )
...
async def oauth_login_redirect(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to redirect to OAuth provider: {str(e)}"
        )
...
async def oauth_callback(
    ...
    if not code or not state:
        raise HTTPException(
            status_code=400, detail="Missing required OAuth parameters (code, state)"
        )
    ...
    try:
        ...
        oauth_manager.logger.error(
            "oauth_callback_processing_error", provider=provider.value, error=str(e)
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to process OAuth callback: {str(e)}"
        )
...
async def list_pending_oauth_users(current_user: User = Depends(get_current_user)):
...
async def reject_oauth_user(
    ...
    if user_data["is_active"]:
        raise HTTPException(status_code=400, detail="Cannot reject active user")
...
@router.get("/config")
async def get_oauth_config(current_user: User = Depends(get_current_user)):
...
async def update_oauth_config(
    config_update: dict, current_user: User = Depends(get_current_user)
):
    ...
    if "auto_register_enabled" in config_update:
        oauth_manager.config.auto_register_enabled = config_update[
    ...
    if "require_admin_approval" in config_update:
        oauth_manager.config.require_admin_approval = config_update[
            ...
            try:
                ...
                if provider in oauth_manager.config.providers:
                    ...
                    if "email_verification_required" in provider_updates:
                        provider_config.email_verification_required = provider_updates[
                            "email_verification_required"
                        ]

                    if "allowed_domains" in provider_updates:
                        provider_config.allowed_domains = provider_updates[
                            "allowed_domains"
                        ]
    ...
    oauth_manager.logger.info(
        "oauth_config_updated", admin_id=current_user.id, updates=config_update
    )
