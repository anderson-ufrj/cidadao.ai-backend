...
class DataIntegrator:
        ...
        if data_type == "contracts":
            return {
                ...
                "value": self._format_currency(
                    data.get("valorInicial", data.get("valor", 0))
                ),
                "contractor": data.get(
                    "nomeRazaoSocialFornecedor", data.get("fornecedor", "N/A")
                ),
                "cnpj": data.get("cnpjContratado", data.get("cnpj", "N/A")),
                "start_date": self._format_date(
                    data.get("dataInicioVigencia", data.get("dataAssinatura", "N/A"))
                ),
                "organ": data.get("nomeOrgao", data.get("orgao", "N/A")),
                "modality": data.get(
                    "modalidadeContrato", data.get("modalidade", "N/A")
                ),
            }
            ...
            return {
                ...
                "value": self._format_currency(
                    data.get("valorDocumento", data.get("valor", 0))
                ),
                "date": self._format_date(
                    data.get("dataDocumento", data.get("data", "N/A"))
                ),
                "beneficiary": data.get(
                    "nomeFavorecido", data.get("favorecido", "N/A")
                ),
            ...
            return {
                ...
                "value": self._format_currency(
                    data.get("valorEstimado", data.get("valor", 0))
                ),
                "modality": data.get(
                    "modalidade", data.get("modalidadeLicitacao", "N/A")
                ),
                "situation": data.get("situacao", data.get("situacaoLicitacao", "N/A")),
                "organ": data.get("nomeOrgao", data.get("orgao", "N/A")),
                "opening_date": self._format_date(
                    data.get("dataAbertura", data.get("data", "N/A"))
                ),
    ...
    async def get_company_overview(self, cnpj: str) -> dict[str, Any]:
        ...
        try:
            ...
            expenses_task = self.search_expenses(
                limit=50
            )  # Expenses don't filter by CNPJ directly
    ...
    def format_data_for_display(self, data: dict[str, Any]) -> str:
        ...
        if not data.get("success"):
            return (
                f"❌ **Erro ao buscar dados**: {data.get('error', 'Erro desconhecido')}"
            )
        ...
        response += (
            f"🕐 **Consultado em**: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
        )
