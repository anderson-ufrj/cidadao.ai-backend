...
help: ## Show this help message
	@echo '$(BLUE)Cidadao.AI - Development Commands$(NC)'
	@echo ''
	@echo 'Usage:'
...
run: ## Run the FastAPI application
	@echo "$(BLUE)Starting Cidadao.AI API...$(NC)"
	$(UVICORN) src.api.app:app --host 0.0.0.0 --port 8000

run-dev: ## Run the application in development mode with hot reload
	@echo "$(BLUE)Starting Cidadao.AI API in development mode...$(NC)"
...
version: ## Show version
	@echo "$(BLUE)Cidadao.AI$(NC) version $(GREEN)1.0.0$(NC)"
...
roadmap: ## Show v1.0 roadmap summary
	@echo "$(BLUE)Cidadao.AI v1.0 Roadmap$(NC)"
	@echo ""
	@cat docs/planning/V1_CHECKLIST.md 2>/dev/null || echo "Roadmap file not found"
	@echo ""
...
v1-report: ## Generate v1.0 progress report
	...
	@echo "# v1.0 Progress Report" > docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "**Date**: $$(date +%Y-%m-%d)" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "## Test Coverage" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@coverage report --skip-empty 2>/dev/null >> docs/reports/progress-$$(date +%Y-%m-%d).md || echo "Run tests first"
	@echo "" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "## Files Changed" >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@git diff --stat >> docs/reports/progress-$$(date +%Y-%m-%d).md
	@echo "$(GREEN)Report saved to docs/reports/progress-$$(date +%Y-%m-%d).md$(NC)"
