...
async def execute_workflow(
    ...
    try:
        ...
        result = await orchestrator.execute_workflow(
            workflow_def.workflow_id, request.initial_data, context
        )
...
async def execute_conditional_workflow(
    ...
    try:
        ...
        execution_path = await orchestrator.execute_conditional_workflow(
            request.workflow_definition, request.initial_data, context
        )
...
async def find_agents_by_capability(
    ...
    try:
                ...
                qualified_agents.append(
                    {
                        ...
                        "match_score": len(
                            set(request.required_capabilities) & set(agent_capabilities)
                        ),
                    }
                )
        ...
        if request.prefer_single_agent and qualified_agents:
            best_agent = await orchestrator.select_best_agent(
            ...
            return {
                ...
                "best_match": {
                    ...
                    "capabilities": (
                        orchestrator._agent_capabilities.get(best_agent.name, [])
                        if best_agent
                        else []
                    ),
                },
...
async def execute_pattern_workflow(
    ...
    try:
        ...
        if pattern == "map_reduce":
            steps = [
                WorkflowStep(step_id="map", agent_name="zumbi", action="analyze"),
                WorkflowStep(step_id="reduce", agent_name="anita", action="aggregate"),
            ]
        elif pattern == "fan_out_fan_in":
            steps = [
                WorkflowStep(step_id="analyze1", agent_name="zumbi", action="analyze"),
                WorkflowStep(
                    step_id="analyze2",
                ...
                WorkflowStep(
                    step_id="analyze3", agent_name="bonifacio", action="policy_analysis"
                ),
            ]
        else:
            steps = [
                WorkflowStep(step_id="step1", agent_name="zumbi", action="analyze")
            ]
        ...
        # Execute
        result = await orchestrator.execute_workflow(
            workflow.workflow_id, data, context
        )

        return {"status": "success", "pattern": pattern, "result": result}
