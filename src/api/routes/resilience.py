...
from src.infrastructure.resilience import bulkhead_manager, circuit_breaker_manager
...
async def get_circuit_breaker_stats(current_user=Depends(get_current_user)):
    """
...
async def get_circuit_breaker_stats_by_service(
    ...
    try:
        ...
        return {"service_name": service_name, "circuit_breaker": stats}
    except Exception as e:
        logger.error(f"Failed to get circuit breaker stats for {service_name}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve circuit breaker statistics for {service_name}",
        )
...
async def reset_circuit_breaker(
    ...
    try:
        ...
        logger.info(
            f"Circuit breaker for {service_name} reset by user {current_user['sub']}"
        )
...
async def reset_all_circuit_breakers(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to reset all circuit breakers"
        )


@router.get("/bulkheads", response_model=dict[str, Any])
async def get_bulkhead_stats(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve bulkhead statistics"
        )


@router.get("/bulkheads/{resource_type}", response_model=dict[str, Any])
async def get_bulkhead_stats_by_resource(
    resource_type: str, current_user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return {"resource_type": resource_type, "bulkhead": stats}
    except Exception as e:
        logger.error(f"Failed to get bulkhead stats for {resource_type}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve bulkhead statistics for {resource_type}",
        )
    ...
    try:
        ...
        return {
            ...
            "recommendations": _generate_health_recommendations(
                circuit_breaker_health, bulkhead_utilization
            ),
        }
    except Exception as e:
        logger.error(f"Failed to get resilience health: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to retrieve resilience health status"
        )


@router.get("/metrics", response_model=dict[str, Any])
async def get_resilience_metrics(current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        return {
            ...
            "aggregate_metrics": {
                ...
                "rejection_rate": (
                    total_rejections / total_requests if total_requests > 0 else 0
                ),
            },
        }
    except Exception as e:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve resilience metrics"
        )


def _generate_health_recommendations(
    circuit_breaker_health: dict[str, Any], bulkhead_utilization: dict[str, Any]
) -> list[str]:
    ...
    high_util_resources = [
        name
        for name, resource in bulkhead_utilization["resources"].items()
