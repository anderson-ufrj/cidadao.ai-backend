...
class Bulkhead:
    ...
    def __init__(self, name: str, config: Optional[BulkheadConfig] = None):
        ...
        logger.info(
            f"Bulkhead '{name}' initialized with type {self.config.bulkhead_type}"
        )
    ...
    async def _execute_with_semaphore(
        self, func: Callable, operation_id: str, start_time: float, *args, **kwargs
    ) -> Any:
        ...
        try:
            async with self._lock:
                ...
                self.stats.max_active_reached = max(
                    self.stats.max_active_reached, self.stats.current_active
                )
        ...
        try:
            ...
            async with self._lock:
                ...
                self.stats.max_queued_reached = max(
                    self.stats.max_queued_reached, self.stats.current_queued
                )

            # Wait for result with timeout
            try:
                result = await asyncio.wait_for(
                    operation["future"], timeout=self.config.timeout
                )
...
class BulkheadManager:
    ...
    def register_default_config(self, resource_type: str, config: BulkheadConfig):
        """
    ...
    def get_bulkhead(
        ...
        if resource_type not in self._bulkheads:
            ...
            bulkhead_config = (
                config or self._default_configs.get(resource_type) or BulkheadConfig()
            )

            self._bulkheads[resource_type] = Bulkhead(resource_type, bulkhead_config)
...
def setup_default_bulkheads():
    ...
    bulkhead_manager.register_default_config(
        "file_operations",
        BulkheadConfig(
            max_concurrent=15, timeout=30.0, bulkhead_type=BulkheadType.SEMAPHORE
        ),
    )
