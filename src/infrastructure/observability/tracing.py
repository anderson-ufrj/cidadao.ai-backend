...
try:
    ...
    ) = AsyncPGInstrumentor = MockInstrumentor
...
class TracingManager:
    ...
    def _setup_exporters(self):
        ...
        if not self.tracer_provider or not OPENTELEMETRY_EXPORTERS:
            if not OPENTELEMETRY_EXPORTERS:
                logger.warning(
                    "OpenTelemetry exporters not available - skipping exporter setup"
                )
            return
    ...
    def _setup_propagators(self):
        ...
        if not OPENTELEMETRY_PROPAGATORS:
            logger.warning(
                "OpenTelemetry propagators not available - skipping propagator setup"
            )
            return
                    ...
                    if include_result and result is not None:
                        try:
                            if isinstance(result, (str, int, float, bool)):
                                ...
                                span.set_attribute(
                                    "function.result_length", len(result)
                                )
                        except Exception as e:
...
async def example_traced_function(investigation_query: str, user_id: str):
    ...
    TraceContext.add_event(
        "investigation.started", {"query_length": len(investigation_query)}
    )
