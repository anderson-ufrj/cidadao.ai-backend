...
async def query_audit_events(
    query_request: AuditQueryRequest, current_user: User = Depends(get_current_user)
):
...
async def get_audit_event_types(current_user: User = Depends(get_current_user)):
...
async def get_audit_severity_levels(current_user: User = Depends(get_current_user)):
...
async def get_audit_dashboard(current_user: User = Depends(get_current_user)):
    ...
    failed_events_filter = AuditFilter(
        success_only=False, start_date=last_24h, limit=10
    )
...
async def create_test_audit_event(current_user: User = Depends(get_current_user)):
    ...
    test_context = AuditContext(
        ip_address="127.0.0.1", user_agent="Test Agent", host="localhost"
    )
    ...
    return {"message": "Test audit event created successfully", "event_id": event.id}
