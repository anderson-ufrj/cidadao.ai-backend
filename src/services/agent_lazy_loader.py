...
class AgentLazyLoader:
    ...
    def __init__(self, unload_after_minutes: int = 15, max_loaded_agents: int = 10):
        """
    ...
    async def _load_agent(self, metadata: AgentMetadata):
        ...
        try:
            ...
            self._stats["avg_load_time_ms"] = (
                avg_load_time * (total_loads - 1) + load_time
            ) / total_loads
    ...
    async def _unload_agent(self, metadata: AgentMetadata):
        ...
        active_instances = [
            key for key in self._instances if key.startswith(f"{metadata.name}_")
        ]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        loaded_count = sum(1 for m in self._registry.values() if m.loaded_class)
