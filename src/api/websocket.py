...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, user_id: str, connection_type: str = "general"
    ):
    ...
    async def subscribe_to_investigation(
    ...
    async def unsubscribe_from_investigation(
        self, websocket: WebSocket, investigation_id: str
    ):
    ...
    async def send_to_investigation(
        self, investigation_id: str, message: WebSocketMessage
    ):
    ...
    async def handle_message(self, websocket: WebSocket, message: dict):
        ...
        try:
            if message_type == "subscribe_investigation":
                ...
                if investigation_id:
                    await self.connection_manager.subscribe_to_investigation(
                        websocket, investigation_id
                    )

            elif message_type == "unsubscribe_investigation":
                investigation_id = data.get("investigation_id")
                if investigation_id:
                    await self.connection_manager.unsubscribe_from_investigation(
                        websocket, investigation_id
                    )
                ...
                if analysis_id:
                    await self.connection_manager.subscribe_to_analysis(
