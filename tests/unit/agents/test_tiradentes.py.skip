    ...
    service.detect_anomalies.return_value = {
        "anomalies": [
            ...
            },
            {
                ...
                "evidence": {"contracts_percentage": 0.35, "value_percentage": 0.42},
            },
        ],
...
def tiradentes_agent():
    ...
    agent = ReporterAgent(default_language="pt", max_report_length=10000)
    return agent
...
class TestTiradentesAgent:
    ...
    @pytest.mark.unit
    async def test_anomaly_threshold_configuration(
        self, mock_data_service, mock_ai_service
    ):
        """Test agent with different anomaly thresholds."""
        with (
            patch("src.agents.tiradentes.DataService", return_value=mock_data_service),
    ...
    async def test_concurrent_investigations(self, tiradentes_agent):
        ...
        contexts = [AgentContext(investigation_id=f"inv-{i}") for i in range(3)]
...
class TestInvestigationRequest:
    ...
    def test_request_creation(self):
        ...
        request = InvestigationRequest(
            investigation_type="corruption_detection",
            data_sources=["contracts", "expenses"],
            parameters={"period": "2024-01-01:2024-12-31", "threshold": 0.8},
            priority="high",
        )
    ...
    def test_request_validation(self):
        ...
        valid_request = InvestigationRequest(
            investigation_type="anomaly_detection", data_sources=["contracts"]
        )
        ...
        with pytest.raises(ValueError):
            InvestigationRequest(
                investigation_type="invalid_type", data_sources=["contracts"]
            )
...
class TestCorruptionIndicator:
    ...
    def test_indicator_risk_scoring(self):
        ...
        high_risk = CorruptionIndicator(
            indicator_type="bid_rigging", risk_level="critical", confidence=0.95
        )

        low_risk = CorruptionIndicator(
            indicator_type="minor_procedural", risk_level="low", confidence=0.60
        )
