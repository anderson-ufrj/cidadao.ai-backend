...
class ModelsClient:
    ...
    def __init__(
        self, base_url: str = None, timeout: float = None, enable_fallback: bool = None
    ):
        """
        ...
        self.enable_fallback = (
            ...
            else settings.models_fallback_local
        ) and LOCAL_ML_AVAILABLE
    ...
    async def detect_anomalies(
        self, contracts: list[dict[str, Any]], threshold: float = 0.7
    ) -> dict[str, Any]:
        """
        ...
        if self.status != ModelAPIStatus.OFFLINE:
            try:
                response = await self.client.post(
                    "/v1/detect-anomalies",
                    json={"contracts": contracts, "threshold": threshold},
                )
                response.raise_for_status()
    ...
    async def analyze_patterns(
        self, data: dict[str, Any], analysis_type: str = "temporal"
    ) -> dict[str, Any]:
        """
    ...
    async def analyze_spectral(
        self, time_series: list[float], sampling_rate: float = 1.0
    ) -> dict[str, Any]:
        """
        ...
        if self.status != ModelAPIStatus.OFFLINE:
            try:
                response = await self.client.post(
                    "/v1/analyze-spectral",
                    json={"time_series": time_series, "sampling_rate": sampling_rate},
                )
                response.raise_for_status()
    ...
    async def _local_anomaly_detection(
        self, contracts: list[dict[str, Any]], threshold: float
    ) -> dict[str, Any]:
    ...
    async def _local_spectral_analysis(
        self, time_series: list[float], sampling_rate: float
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "periodic_patterns": [
                {"frequency": 0.5, "period": "semi-annual", "strength": 0.8}
            ],
            "source": "local_fallback",
        }
