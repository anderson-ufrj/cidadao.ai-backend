...
class TestMLTrainingPipeline:
    ...
    async def test_model_versioning(self, pipeline, sample_data):
        ...
        with (
            ...
            result1 = await pipeline.train_model(
                model_type="anomaly", algorithm="isolation_forest", X_train=X_train
            )
            ...
            assert (
                pipeline.model_registry["anomaly_isolation_forest"][
                    "versions"
                ].__len__()
                == 2
            )
    ...
    async def test_load_model(self, pipeline, sample_data):
        ...
        with (
            patch("joblib.load", return_value=model_package),
            patch.object(
                pipeline,
                "model_registry",
                {
    ...
    async def test_promote_model(self, pipeline):
        ...
        with patch(
            "src.infrastructure.cache.redis_client.get_redis_client"
        ) as mock_redis:
    ...
    async def test_compare_models(self, pipeline):
        ...
        with patch.object(pipeline, "load_model") as mock_load:
            ...
            results = await pipeline.compare_models(
                [("model1", 1), ("model2", 2)], test_data, test_labels
            )
...
class TestABTestingFramework:
    ...
    async def test_create_ab_test(self, ab_framework):
        ...
        with (
            patch.object(training_pipeline, "load_model") as mock_load,
            patch(
                "src.infrastructure.cache.redis_client.get_redis_client"
            ) as mock_redis,
        ):
